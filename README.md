...
1. Clone the plugin project by running `git clone https://github.com/Pardus-Kurumsal/lider-ahenk-password-plugin.git`.
2. Open Eclipse and import the plugin project into Eclipse as 'Existing Maven Projects'.
3. Navigate to _lider-ahenk-password-plugin/scripts_ directory and run `build-plugin.sh`.
...
1. Type `feature:repo-add  mvn:tr.org.liderahenk/lider-password-feature/1.0.0/xml/features` on Karaf shell. This will add plugin repository to the Karaf instance.
2. Again on Karaf shell, run `feature:install lider-password` to install and run plugin bundles.
